...
do
  if echo 'show interfaces' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |egrep '(Socket|Ethernet)' >/dev/null 2>&1; then
    ...
    if [ $version == "V7.1" ]; then
      ...
      model_type=$(echo 'show parameter model' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |cut -d"=" -f2 | sed -e 's/^[[:space:]]*//')
      if [ $model_type == "tiny" ]; then
      ...
      if [ $bump_count == 4 ]; then
        if [ $model_type == "tiny" ]; then
          soc_a=$(echo 'show interfaces select pci_address' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep Socket |grep External |awk '{print $'$pos_pciaddr'}' |awk 'FNR == 1 {print}'|rev |cut -d":" -f2)
          soc_b=$(echo 'show interfaces select pci_address' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep Socket |grep External |awk '{print $'$pos_pciaddr'}' |awk 'FNR == 2 {print}'|rev |cut -d":" -f2)
          soc_c=$(echo 'show interfaces select pci_address' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep Socket |grep Internal |awk '{print $'$pos_pciaddr'}' |awk 'FNR == 1 {print}'|rev |cut -d":" -f2)
          soc_d=$(echo 'show interfaces select pci_address' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep Socket |grep Internal |awk '{print $'$pos_pciaddr'}' |awk 'FNR == 2 {print}'|rev |cut -d":" -f2)
  ...
  if [ ! -z $bitw_port_1 ]; then
    if [ "$bitw_port_1" != "tree)" ]; then
      if [ $version == "V7.1" ]; then
        ...
        bitw_port_1_adminstatus=$(echo 'show interfaces select interface system_interface uid admin_status' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep $bitw_port_1_index |awk '{print $'$pos_admin_status'}')
  ...
  if [ ! -z $bitw_port_1_adminstatus ]; then
    ...
    if [ ! -z $bitw_port_2 ]; then
      if [ $version == "V7.1" ]; then
        ...
        bitw_port_2_adminstatus=$(echo 'show interfaces select interface system_interface uid admin_status' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep $bitw_port_2_index |awk '{print $'$pos_admin_status'}')
  ...
  if [ ! -z $bitw2_port_1 ]; then
    if [ "$bitw_port_1" != "tree)" ]; then
      if [ $version == "V7.1" ]; then
        ...
        bitw2_port_1_adminstatus=$(echo 'show interfaces select interface system_interface uid admin_status' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep $bitw2_port_1_index |awk '{print $'$pos_admin_status'}')
  ...
  if [ ! -z $bitw2_port_1_adminstatus ]; then
    ...
    if [ ! -z $bitw2_port_2 ]; then
      if [ $version == "V7.1" ]; then
        ...
        bitw2_port_2_adminstatus=$(echo 'show interfaces select interface system_interface uid admin_status' | sudo /opt/stm/target/pcli/stm_cli.py $id:$pass@localhost |grep $bitw2_port_2_index |awk '{print $'$pos_admin_status'}')
...
function check_bypass_status()
{
  ...
  if [ "$bump_type" == "cooper" ]; then
    if bpctl_util "$real_port_1_pci" get_bypass |grep non-Bypass >/dev/null 2>&1; then
      bitw1_cooper_bypass='normal'
    else
      bitw1_cooper_bypass='bypass'
    fi
    if [ $realint_count == "4" ]; then
      if bpctl_util "$real_port_3_pci" get_bypass |grep non-Bypass >/dev/null 2>&1; then
        bitw2_cooper_bypass='normal'
      else
        bitw2_cooper_bypass='bypass'
      fi
    else
...
#  if [ "$bump_type" == "fiber" ]; then
    if [ -d /opt/stm/bypass_drivers/silicom_bpdrv/$bp_ver ]; then
      cd /opt/stm/bypass_drivers/silicom_bpdrv/$bp_ver
      network_bypass=$(lsmod |grep bpctl_mod |awk '{ print $1 }')
      if [ -z $network_bypass ]; then
	      echo "Insert bpctl module" | awk '{ print strftime(), $0; fflush() }' >> /var/log/stm_bypass.log
        sudo bpctl_start
      fi
    fi
#  fi
...
# pre-check
sudo bpctl_util all set_std_nic off >/dev/null 2>&1
