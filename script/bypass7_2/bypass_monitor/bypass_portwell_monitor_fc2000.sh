...
fiber_bump1_in_use="slot0"
fiber_bump2_in_use="slot0"
#COSD304 : 2u 10cores
#CAR3070 : 2u 4cores
board="COSD304"
...
function enable_portwell_bypass()
{
        ...
        if [ $1 -eq 1 ]; then
                if [ "$bump_type" == "cooper" ]; then
                        ...
                        for fiber_bump1_slot_bypass in $(cat /etc/stm/system_fiber_slots |egrep "$fiber_bump1_in_use"); do
			# for ((i=0; i<${#fiber_slot1_bypass0[@]}; i++)); do                                        
                        if [ -e /sys/class/misc/caswell_bpgen2/${fiber_bump1_slot_bypass:0:5}/${fiber_bump1_slot_bypass:6} ]; then
                        # if [ -e /sys/class/misc/caswell_bpgen2/slot0/bypass0 ]; then
        ...
        if [ $1 -eq 2 ]; then
			...
			# for ((i=0; i<${#fiber_slot2_bypass0[@]}; i++)); do
                        for fiber_bump2_slot_bypass in $(cat /etc/stm/system_fiber_slots |egrep "$fiber_bump2_in_use"); do                                        
                        if [ -e /sys/class/misc/caswell_bpgen2/${fiber_bump2_slot_bypass:0:5}/${fiber_bump2_slot_bypass:6} ]; then
                        # if [ -e /sys/class/misc/caswell_bpgen2/slot0/bypass1 ]; then
...
function bypass_status_check()
{
		...
		if [ "$fiber_bump1_in_use" = "None" ]; then
		...
		if [ "$fiber_bump2_in_use" = "None" ]; then
