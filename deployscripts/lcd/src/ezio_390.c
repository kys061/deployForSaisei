...
#ifdef EZIO_390_CONFIG
void print_help(void)
{
	printf("\n");

	printf("ezio_390_api - The CASwell EZIO sample API\n");
	printf("Usage: ezio_390_api -d [device] -[command]\n");
	printf("[device]\n");
...
int get_key(unsigned int fd)
{
	...
	while (1) {
		...
		/* Switch the Read command */
                //printf("key=%x-%x\n",buf[0],buf[1]);
		switch (buf[1]) {
		case 0x01:	/* Up button was received */
			if (old_status != 0x01) {
			...
			break;
		case 0x02:	/* Down button was received */
			if (old_status != 0x02) {
			...
			break;
		case 0x04:	/* Enter buttonn was received */
			if (old_status != 0x04) {
			...
			break;
		case 0x08:	/* ESC button was received */
			if (old_status != 0x08) {
