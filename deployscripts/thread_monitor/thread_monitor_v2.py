...
def check_subprocess_data(sub):
    ...
    if result is '':
        ...
        try:
            raise Exception('TimeOutError, No response from stm')
        except Exception as e:
            logger.error('{}'.format(e))
            pass
    else:
...
def check_stm_status():
    ...
    if version == "V7.3":
        if check_subprocess_data(subprocess_open(
            get_command(r"show int", "| egrep '(Socket|[eE]thernet)' |awk '{print $1\",\"$6}'"), 10)[0]):
        ...
        if check_subprocess_data(subprocess_open(
            get_command(r"show int", "| egrep '(Socket|[eE]thernet)' |awk '{print $1\",\"$4}'"), 10)[0]):
            raw_data = subprocess_open(
                get_command(r"show int", "| egrep '(Socket|[eE]thernet)' |awk '{print $1\",\"$4}'"), 10)[0]
            is_stm_started = True
            return check_data_error(raw_data)
...
def check_interface_thread():
    global thread_chk_count, is_stm_started, apache_restart_count, is_thread_status
    ...
    if check_subprocess_data(subprocess_open(
        ...
        if int(parameter) == 0:
            ...
            if check_subprocess_data(subprocess_open(
                ...
                try:
                    _interfaces = subprocess_open(
                        get_command(r"show int", r"| grep '[eE]thernet' |grep 'external' |awk '{{ print $1 }}'"), 10)[0].split('\n')
                    try:
                        _interfaces_iterator = iter(_interfaces)
                    except TypeError as e:
                        logger.error('_interfaces is not iterable, {}'.format(e))
                    else:
            ...
            if check_subprocess_data(subprocess_open(
                ...
                try:
                    _interfaces = subprocess_open(
                        get_command(r"show int", r"| grep '[eE]thernet' |awk '{{ print $1 }}'"), 10)[0].split('\n')
                    try:
                        _interfaces_iterator = iter(_interfaces)
                    except TypeError as te:
            ...
            thread_count = 10
            logger.info('cores_per_interface is 0, Please check parameters...')
            logging_line()

        if thread_chk_count > thread_count * MUL:
            logger.info('No stm threads : start rebooting now...')
            # is_stm_started = False
            is_thread_status = False
            logging_line()
            # reboot_system()
        else:
...
def main():
    global version
    global is_stm_started, is_stm_err, is_thread_status
