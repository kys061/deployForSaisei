...
platform=$(dmidecode |grep "Chassis Information" -A10 |grep Type | cut -d ":" -f2 |  tr -d ' ')
...
function cpFilesToTarget(){
  ...
  cp ${bypass_deploy_path}${bypass7_3_cooper_driver} ${bypass7_2_target}
  if [ $? -eq 0 ]; then
      echo -e "${bypass_deploy_path}${bypass_cooper_driver} is coping in ${bypass_target}"
  	echo -e "${bypass_deploy_path}${bypass_cooper_driver} is coping in ${bypass7_2_target}"
  	echo -e "${bypass_deploy_path}${bypass7_3_cooper_driver} is coping in ${bypass7_2_target}"
      sleep $sleep_time
      echo -e "${red}...OK!${NC}"
  fi
  cp ${bypass_deploy_path}${bypass_fiber_driver} ${bypass_target}
  cp ${bypass_deploy_path}${bypass_fiber_driver} ${bypass7_2_target}
  cp ${bypass_deploy_path}${bypass7_3_fiber_driver} ${bypass7_2_target}
  if [ $? -eq 0 ]; then
      echo "# ${bypass_deploy_path}${bypass_fiber_driver} is coping in ${bypass_target}"
  	echo -e "${bypass_deploy_path}${bypass_fiber_driver} is coping in ${bypass7_2_target}"
  	echo -e "${bypass_deploy_path}${bypass7_3_fiber_driver} is coping in ${bypass7_2_target}"
      sleep $sleep_time
      echo -e "${red}...OK!${NC}"
  fi
...
function setGrubConfig(){
  ...
  # change grub setting
  sed -i "/^GRUB_CMDLINE_LINUX_DEFAULT=/s/hugepages=512\"/hugepages=512 isolcpus=1-2 biosdevname=0\"/g" $grub_path
  if [ $? -eq 0 ]; then
  	update-grub
    echo "# changed grub setting"
    sleep $sleep_time
    echo -e "${red}...OK!${NC}"
...
function setOtherConfig(){
  ...
  if [ $? -eq 0 ]; then
      echo "# changed run_on_boot as true!"
      sleep $sleep_time
      echo -e "${red}...OK!${NC}"
  fi
